...
export const POST: RequestHandler = async (event) => {
  ...
  if (!body.lesson_id || !body.title || !body.questions) {
    return new Response(JSON.stringify({ error: { code: 'BAD_REQUEST', message: 'Missing lesson_id, title, or questions.' } }), { status: 400, headers: { 'content-type': 'application/json' } });
  }

  const result = await proxyToTeacher(userId, '/create-lesson-assessment', undefined, 'POST', body);
  if (result.error) {
