...
export const POST: RequestHandler = async (event) => {
  ...
  if (!body.submission_id || !body.answers) {
    return new Response(JSON.stringify({ error: { code: 'BAD_REQUEST', message: 'Missing submission_id or answers.' } }), { status: 400, headers: { 'content-type': 'application/json' } });
  }

  const result = await proxyToTeacher(userId, '/grade-submission', undefined, 'POST', body);
  if (result.error) {
