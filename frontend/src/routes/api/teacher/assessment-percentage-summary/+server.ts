...
export const GET: RequestHandler = async (event) => {
  ...
  const session_term_id = url.searchParams.get('session_term_id');
  const subject_id = url.searchParams.get('subject_id');
  if (!session_term_id || !subject_id) {
  ...
  const result = await proxyToCoreApi(userId, '/teacher/assessment-percentage-summary', { session_term_id, subject_id });
  if (result.error) {
