...
export const POST: RequestHandler = async (event) => {
	...
	if (!body || !body.id || !body.session_name || !body.term_id) {
		return new Response(
			JSON.stringify({ error: { code: 'VALIDATION_ERROR', message: 'id, session name, and term are required' } }),
			{ status: 400, headers: { 'content-type': 'application/json' } }
		);
	}

	const proxy = adminProxy(user);
	const result = await proxy('/edit-session-term', undefined, 'POST', {
